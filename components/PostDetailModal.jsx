  ...
  const loadPostDetails = async () => {
    try {
      ...
      if (postData.authorType === 'user') {
        ...
        if (authorDoc.exists()) {
          setAuthor({ id: authorDoc.id, ...authorDoc.data(), type: 'user' });
        }
      } else if (postData.authorType === 'club') {
        const clubDoc = await getDoc(doc(db, 'clubs', postData.authorId));
        if (clubDoc.exists()) {
          setAuthor({ id: clubDoc.id, ...clubDoc.data(), type: 'club' });
        }
      }
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (isLiked) {
        ...
        setPost(prev => ({
          ...prev,
          likes: prev.likes.filter(id => id !== auth.currentUser.uid)
        }));
        setIsLiked(false);
      } else {
        await updateDoc(postRef, {
            ...
            <Pressable style={styles.authorSection} onPress={handleAuthorPress}>
              <View style={[styles.authorAvatar, { backgroundColor: theme.uiBackground }]}>
                {author?.profilePhoto || author?.image ? (
                  <Image 
                    source={{ uri: author.profilePhoto || author.image }} 
